...
export default async function reconstituteFileFromOnChainBuffer({
    ...
    if (fs.existsSync(decompressedFilePath)) {
        console.log(`Domain: ${domainName}, Buffer version: ${bufferVersion}, ${decompressedFilePath} file is already exist`);
    } else {
        console.log('decompressedFilePath', decompressedFilePath)

        // Trick, because we do not know the name of the directory under the compressed file, create the directory we want to see
        // and use --strip-components option
        const command = `mkdir ${decompressedFilePath} && tar -xvf ${compressedFilePath} -C ${decompressedFilePath} --strip-components 1`;
